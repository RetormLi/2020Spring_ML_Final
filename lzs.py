...
# search = GridSearchCV(estimator, param_grid, n_jobs=-1, verbose=10)
...
# Best parameters found by grid search are: {'lambda_l1': 0.001, 'max_depth': 6, 'num_leaves': 31}
# Best score: 0.6593003002132404

if not os.path.isfile('model.txt'):
    train_set = lgbm.Dataset(data=train_X, label=train_y)
    model = lgbm.train(params=params, train_set=train_set, categorical_feature=[
                       'weekday', 'time_slot', 'time_point', 'id_road'])
...
noLabel = preprocess_tti_no_label(pd.read_csv(
    'G:/data/datas/traffic1/toPredict_noLabel.csv'))
noLabel['date'] = noLabel['date'].astype(str)
test_y['date'] = test_y['date'].astype(str)
result = noLabel.merge(
    test_y, on=['id_road', 'date', 'time_slot', 'time_point'])
