...
def gps_str_to_list(gps_data):
    ...
    for i in range(len(gps_list)):
        ...
        day = int(datetime.datetime.strptime(
            daylist[0], '%Y/%m/%d').strftime("%w"))
        ...
        gps_list[i] = [float(gps_list[i][0]),
                       ...
                       float(gps_list[i][3]),
                       str(daylist[1].split(":")[0])+":" +
                       str(daylist[1].split(":")[1]),
                       daylist[0],  # 日期
                       day,  # 周日-周六：0-6
                       time]  # 归一化的当日时间
    return gps_list
...
def preprocess_tti(tti_data):
    ...
    tti_data['weekday'] = (tti_data['time'].apply(lambda x: int(
        datetime.datetime.strptime(str(x).split()[0], '%Y-%m-%d').strftime("%w")))+1)/7
    tti_data['daytime'] = (tti_data['time'].apply(lambda x: int(str(x).split()[1].split(":")[0]))*3600 + tti_data['time'].apply(
        lambda x: int(str(x).split()[1].split(":")[1]))*60 + tti_data['time'].apply(lambda x: int(str(x).split()[1].split(":")[2])))/86400
...
rec_road = {
    ('276183', '276184'): [22.591219, 22.586757, 114.017324, 114.028209],
    ('275911', '275912'): [22.603728, 22.586757, 114.014567, 114.017324],
    ('276240', '276241'): [22.610348, 22.603514, 114.015564, 114.023718],
    ('276264', '276265'): [22.606308, 22.59149, 114.026608, 114.034633],
    ('276268', '276269'): [22.608082, 22.599193, 114.031416, 114.039827],
    ('276738', '276737'): [22.610044, 22.602214, 114.023188, 114.031664]
}
...
def get_block_rec(block):
    # 上下左右
    block_rec = [max([i[1] for i in block]), min([i[1] for i in block]), min(
        [i[0] for i in block]), max([i[0] for i in block])]
    return block_rec
    ...
    for i in range(len(x)):  # i是每条轨迹
        ...
        while j < len(x[i]):  # j是每条轨迹上的每个点
            ...
            for block in roads.keys():
                if in_block(gps, roads[block], rec_road[block]):
                    road = get_road(x[i][j][3], block)  # 第j个点的路段
                    day = x[i][j][5][:4]+"-"+x[i][j][5][5:7] + \
                        "-"+x[i][j][5][8:]+" "+x[i][j][4][:-1]+"0"
                    ...
                    if (road != temp1 or day != temp2) and temp1 != '' and temp2 != '':
                        ...
                        idx = [i for i in y[y['id_road'].values ==
                                            temp1].index if i in y[y['day'].values == day].index]
...
# g1就是用gps_str_to_list跑出来的前1w条测试集的数据
g1 = pd.read_csv('G:/data/pred_gps_0-10000-1.csv')
